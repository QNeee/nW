...
export const OnPendings = () => {
    ...
    </FindDiv>{!loading && filteredFriends.length > 0 ? filteredFriends.map(item => <FriendsDiv key={item._id}><p><Link to={`/home/profile/${item.find}`}>{item.nickName}</Link></p><p><img src={item.avatarURL ? item.avatarURL : noAvatar} alt={item.nickName} width='250' height='250' /></p>
